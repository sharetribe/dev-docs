...
You'll notice we mention secure and trusted contexts in the above
comparison. Some requests must be made from a _trusted context_, meaning
the SDK must be initialized with both the client ID and client secret.
As a result, all Integration SDK requests are performed within a trusted
context.

Additionally, some requests will require a _secure context_, meaning
they need to be made from a server-side environment. An easy way to do
this is to run a small server from which to make these types of
requests.
...
from a trusted context (i.e. on the server). In order to do this, you'll
need to log in using the SDK and perform a token exchange to retrieve a
trusted token.
