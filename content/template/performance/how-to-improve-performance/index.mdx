...
We recommend using
[code-splitting](/template/routing/code-splitting-in-template/) to
improve initial page load times. You can further optimise performance by
moving components only used on specific pages to page directories (with
page directories being the directories you can find under
`src/containers/`). Code located in the `src/components` directory is
loaded into the `main.bundle.js` file served to the client on the
initial page load.
...
You can read more
[in the code splitting article](/template/routing/code-splitting-in-template/).
