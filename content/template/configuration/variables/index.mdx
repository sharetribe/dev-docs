...
The order of the `images` array on a listing determines the order the
images are displayed in, and the first image is used as the listing's
cover photo. When creating or editing a listing, authors can reorder
images by dragging and dropping them, or with keyboard commands for
accessibility.
