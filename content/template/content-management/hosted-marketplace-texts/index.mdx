...
The app calls the **renderApp** function, which renders `<ServerApp>` in
_src/app.js_. The hosted marketplace text file from Asset Delivery API
is passed as a prop to `<ServerApp>`:
...
using the `sdk.assetByVersion` method. {<h3></h3>}

The browser loads the initial data using a load data call.

{<h3></h3>}

Hydrate the `<ClientApp>` and pass marketplace texts as props. Hosted
...
The browser renders the `<ClientApp>` and passes marketplace texts as
props.
