...
sidebarTitle: Hosted marketplace texts
description:
...
<FileTree>
  <FileTree.Folder name="server" defaultOpen>
        ...
        <FileTree.File name="dataLoader.js" active />

  </FileTree.Folder>
</FileTree>

```jsx filename="dataLoader.js"
...
3. SSR: `fetchAppAssets` thunk fetches the latest version of
   ...
   `sdk.assetByAlias`. This ensures that the server-side rendering has
   the most recent version of asset to render the page
4. SSR: make a `loadData` call if the route specifies it:
...
10. Browser: hydrate the `<ClientApp>` and pass marketplace texts as
    props.
