...
- **Discount based on booking length.** An example: daily price is $20,
  weekly price $70, and monthly price \$200. Another example: 20%
...
requires both an amount and a currency.

`new Money(amount: Number, currency: String)`

When working with the [APIs](https://www.sharetribe.com/api-reference/) and in
the Template, it is important to ensure that the amount is an integer, as
