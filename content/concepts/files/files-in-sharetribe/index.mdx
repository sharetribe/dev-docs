...
attached to another resource – in the first release, to a message via
the `message.publicFileAttachments` attribute.

Once a file has been attached to another resource, it is visible as a
`file` resource according to the scope of the corresponding
...
specific use cases for files in your marketplace. The first file related
Sharetribe Web Template release makes it possible to enable attaching
files to messages based on listing type.
...
A `fileAttachment` is then a related resource of another resource, for
example a message. In this structure, it is possible for a `file` to
have `fileAttachment` links to multiple different resources.

When querying the resource, you'll need to include both the file
attachments resource and the nested file relationship if you want to
retrieve both.
...
(question being answered: how does Sharetribe make sure attackers don't
use files for fraud)
