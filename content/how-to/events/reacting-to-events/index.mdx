...
description: This guide demonstrates how to build an integration that reacts to
...
[Integration API SDK](/concepts/api-sdk/js-sdk/) ready. If you have not yet used
...
- in case your marketplace has the
  [listing approval feature](/concepts/listings/requiring-approval/) on, a
...
const analyzeEvent = (event) => {
  const { resource: listing, previousValues, eventType } = event.attributes;
