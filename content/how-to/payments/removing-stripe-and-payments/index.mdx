...
needs to **already exist in your marketplace**. If you want to add a new
transaction process without Stripe actions instead of replacing your
existing one, you first need to add an unmodified version of it to your
...
### Remove Stripe checks from EditListingWizard and MakeOfferForm
...
component has a limitation that providers cannot submit their offer if
they don't have a Stripe account. You will need to remove the references
to `stripeConnected` in the form to remove this restriction.
...
### Hide/remove Stripe related pages
