...
`transactionProcessBooking.js` file to correspond to the new process. If
you are creating a parallel booking process and want to allow providers
to choose between two processes for their listings, you will need to
create a new transaction process file and import it in `transaction.js`.
...
### Update state graph to match the new transaction process
...
`InboxPage.stateDataNegotiation.js`), you will need to export a uniquely
named `getStateDataFor...` function from that file.
