...
[Tutorial introduction](https://www.sharetribe.com/docs/tutorial/), as
this step requires having your code hosted in a Git repository.
...
Find the Build command, and set it to this value:

```
yarn install --production=false; yarn build
```

![Render build command](./render-build-command-tutorial.png)

You do not need to touch any other settings (such as environment,
branch, and start command), and select the free plan. Before clicking on
"create web service", click on "Advanced" to enter the required
environment variables.

There is one environment variable you need to add through the browser
interface, related to Node version. Add the following key and value:

```
NODE_VERSION: 22.22.0
```

![Node_version environment variable](./render-env-variable-node-version.png)

For the other environment variables, you can choose to enter them
one-by-one through the browser interface:
...
- `REACT_APP_CSP`
  ...
  [this article](/template/security/how-to-set-up-csp-for-template/).<br />
...
In [the next part of this tutorial](/tutorial/new-wizard-tab/), we'll go
through how to add new tabs to the listing wizard.
