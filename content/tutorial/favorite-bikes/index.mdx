...
```jsx filename="OrderPanel.js" {10} showLineNumbers{468}
...
```js filename="ListingPageCarousel.js" {9, 13} showLineNumbers{72}
import {
  LoadingPage,
  ErrorPage,
  priceData,
...
```jsx filename="ListingPageCarousel.js" {2} showLineNumbers{602}
...
```jsx filename="ListingPageCarousel.js" {5} showLineNumbers{131}
    onInitializeCardPaymentData,
    config,
    routeConfiguration,
    showOwnListingsOnly,
...
export const FavoriteListingsPageComponent = props => {
  const { listings, pagination, queryInProgress,
  queryFavoritesError, queryParams, scrollingDisabled, intl } = props;
