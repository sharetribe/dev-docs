...
This tutorial uses the following no-code marketplace configurations:
![Design > Layout > Listing page image layout: **Image carousel with thumbnails**](./listing-page-image-layout.png)
  ...
  <FileTree.Folder name="src" defaultOpen>
    <FileTree.Folder name="components" defaultOpen>
      <FileTree.Folder name="OrderPanel" defaultOpen>
        <FileTree.File name="OrderPanel.js" />
        <FileTree.File name="OrderPanel.module.css" />
      </FileTree.Folder>
    </FileTree.Folder>
  </FileTree.Folder>
</FileTree>
...
```jsx filename="OrderPanel.js" / Button/ /SecondaryButton/
import {
  ModalInMobile,
  PrimaryButton,
  AvatarSmall,
...
```jsx filename="OrderPanel.js" {5-6} showLineNumbers{294}
...
<details>
<summary>
Why does the `isFavorite` definition contain question marks?
</summary>

Above, we define the `isFavorite` constant like this:
...
```css filename="OrderPanel.module.css"
.favoriteButton {
...
Start by locating the ListingPage.shared.js file:

<FileTree>
  <FileTree.Folder name="src" defaultOpen>
    <FileTree.Folder name="containers" defaultOpen>
      <FileTree.Folder name="ListingPage" defaultOpen>
        <FileTree.File name="ListingPage.shared.js" active />
...
```jsx filename="ListingPage.shared.js"
...
```js filename="ListingPageCarousel.js" {9, 14} showLineNumbers{40}
import {
  LoadingPage,
  ErrorPage,
  handleContactUser,
...
To dispatch Redux actions from a component, we use the
[useDispatch](https://react-redux.js.org/api/hooks#usedispatch) hook.

In _ListingPageCarousel.js_, there are two components. The outer one,
`ListingPage`, is responsible for wiring up Redux: it calls
`useDispatch()` to get a reference to the store's dispatch function, and
then creates a separate callback for each Redux action using React's
[useCallback](https://react.dev/reference/react/useCallback) hook. Those
callbacks are then passed as props to the inner `ListingPageComponent`,
which does the actual rendering.

We will add `onUpdateFavorites` in the same way. Find the `ListingPage`
component — it starts after the `ListingPageComponent` definition — and
add the new `useCallback` after the other existing ones.

Add the new callback after the last existing `useCallback` in
`ListingPage` (after the `onFetchTimeSlots` callback):

```jsx filename="ListingPageCarousel.js" {7-8} showLineNumbers{440}
...
const onUpdateFavorites = useCallback(
...
Then pass it as a prop to `ListingPageAccessWrapper` inside the `return`
of `ListingPage`, alongside the other existing props:

```jsx filename="ListingPageCarousel.js" {3} showLineNumbers{451}
return (
  <ListingPageAccessWrapper
    {...props}
    onUpdateFavorites={onUpdateFavorites}
    {/* ...all other existing props stay as they are */}
  />
```
...
Now that the _ListingPage_ creates _onUpdateFavorites_ and passes it as
a prop, _ListingPageCarousel_ props can receive and use it. Let’s add it
to the
...
```jsx filename="ListingPageCarousel.js" {5} showLineNumbers{91}
...
```jsx filename="ListingPageCarousel.js" {7-8}
<OrderPanel
...
<details>
<summary>
How does the network call propagate to the button?
</summary>
...
- when the sdk call succeeds, it returns the updated _currentUser_
  resource
- the Redux action then calls _updateProfile_, which updates the
  _currentUser_ in the store with the updated resource
...
_FavoriteListingsPage_, and create three new files:

<FileTree>
  <FileTree.Folder name="src" defaultOpen>
    <FileTree.Folder name="containers" defaultOpen>
      <FileTree.Folder name="FavoriteListingsPage" defaultOpen>
        <FileTree.File name="FavoriteListingsPage.js" />
        <FileTree.File name="FavoriteListingsPage.duck.js" />
        <FileTree.File name="FavoriteListingsPage.module.css" />
      </FileTree.Folder>
    </FileTree.Folder>
  </FileTree.Folder>
</FileTree>
...
<details>
<summary>
`FavoriteListingsPage.js` explained
</summary>
...
to get the container state into constants for ease of use.
...
Finally, we export the component as the default export.
...
<details>
<summary>
`FavoriteListingsPage.duck.js` explained
</summary>
...
Then, we start creating the different elements using
[Redux Toolkit](https://redux-toolkit.js.org/):

- [State Slice](https://redux-toolkit.js.org/api/createSlice)
- [Reducer](https://redux-toolkit.js.org/api/createReducer)
- [Thunks](https://redux-toolkit.js.org/api/createAsyncThunk)

We define initial state and a helper function _resultIds_:
...
We then create a `slice` where we define the initial state, an object of
reducer functions, and a "slice name". The slice automatically generates
action creators and action types that correspond to the reducers and
state. The slice handles three different outcomes:
...
const favoriteListingsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        queryFavoriteListingsThunk.fulfilled,
        (state, action) => {
...
Finally, we create our thunk to call the SDK. The main SDK call we want
to make on this page is fetching listings based on the user's favorite
listings.
...
- Make SDK call
- For a successful SDK response, handle the response data and return a
  **fulfilled** status – this again sets a selection values in state,
  which the page can use to stop loading indication and display the data
- In case of an error, return `rejectWithValue` and a **rejected**
  status, which will store a selection of values in state that the page
  can use to stop loading indication and display an error message.

```jsx
export const queryFavoriteListingsPayloadCreator = (
  queryParams,
...
<FileTree>
  <FileTree.Folder name="src" defaultOpen>
    <FileTree.Folder name="containers" defaultOpen>
      <FileTree.File name="reducers.js" active />
    </FileTree.Folder>
  </FileTree.Folder>
</FileTree>
...
```jsx filename="src/containers/reducers.js" {1, 5}
...
<FileTree>
  <FileTree.Folder name="src" defaultOpen>
    <FileTree.Folder name="containers" defaultOpen>
      <FileTree.File name="pageDataLoadingAPI.js" />
    </FileTree.Folder>
...
```jsx filename="pageDataLoadingAPI.js"
...
```jsx filename="src/routing/routeConfiguration.js"
...
Now, when you navigate to `localhost:3000/favorites`, you can see your
favorited listings.
...
<FileTree>
  <FileTree.Folder name="src" defaultOpen>
    <FileTree.Folder name="containers" defaultOpen>
      <FileTree.Folder name="UserNav" defaultOpen>
        <FileTree.File name="userNav.js" />
      </FileTree.Folder>
    </FileTree.Folder>
  </FileTree.Folder>
</FileTree>
