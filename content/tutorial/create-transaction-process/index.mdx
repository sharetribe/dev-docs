...
description:
  This tutorial describes how to create a new transaction process and
  how to take it into use in the Sharetribe Web Template.
---
...
[push payments](/concepts/payments/payment-methods-overview/#push-payment-flow)
...
```clojure filename="biketribe-instant-booking/process.edn"
...
```diff filename="src/config/configListing.js"
...
```diff filename="src/util/configHelpers.js"
...
Since our new process is very similar to the default one, we can
...
```jsx filename="src/transactions/transactionProcessInstantBooking.js"
...
```diff filename="src/transactions/transactionProcessInstantBooking.js"
export const graph = {
...
```diff filename="src/transactions/transaction.js"
...
Then, we will add our new instant process to the array of supported
processes, and allow it to handle all time based unit types. Our new
listing type uses `unitType: day`, and adding all time-based unit types
in this configuration allows us to create other listing types that use
the same process with other unit types.
...
Finally, we need to update the _isBookingProcess_ helper. We need to add
the new process name to the list used to determine which processes
handle bookings, since we are adding a booking related process.

This helper is used in multiple places in the template, such as in the
ListingCard and OrderPanel components, and on checkout page, listing
editing page, and transaction page.
...
you will also need to ensure that the newly created transaction process
is properly checked at the time of deletion for states that contain
Stripe-related payment processing. This is handled in
...
+ stripeRelatedStatesForBiketribeInstantBookings = [
+     stripeRelatedStatesForBiketribeInstantBookings = [
...
+ const ongoingBiketribeInstantBookingsWithIncompletePaymentProcessing = () =>
+     sdk.transactions.query({
+       processNames: 'negotiated-booking',
+       states: stripeRelatedStatesForBiketribeInstantBookings.join(','),
+     });

 Promise.all([
    ...
+     ongoingBiketribeInstantBookingsWithIncompletePaymentProcessing(),
  ])
  ...
};
```
...
Finally, we added error handling to reduce API calls and clearly
