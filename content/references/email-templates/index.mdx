...
- locale
  - either provided as a string or derived from an asset
...
```handlebars
{{set-timezone 'Helsinki/Europe'}}
```

This helper applies a timezone to date/time text strings in the emails.
It is used by the `format-text` function - wrapped in `format-day-time`
in this case. It needs to be provided in
[TZ timezone database](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones)
format.

This is consistently used transaction process email templates, where the
timezone exists as a parameter of the listing's availability plan, for
example:
...
**Email texts**. These texts are stored in the backend at the path
`content/email-texts.json`.

In the email templates, these texts are accessed using
[helper functions](#custom-helpers) which use the text keys to retrieve
the corresponding values and insert or replace the content in the
templates.
...
This header uses the [`t helper`](#t) to look up the key
...
- The listing `availability-plan` property `timezone` is in TZ timezone
